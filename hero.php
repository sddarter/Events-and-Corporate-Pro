...
function corporate_page_title() {
	...
	if ( is_home() || is_archive() || is_category() || is_tag() || is_tax() || is_search() || is_page_template( 'page_blog.php' ) || ( tribe_is_event() && is_single() ) ) {
	...
	} elseif ( class_exists( 'single-tribe_events' ) ) {

		genesis_markup( array(
			'content' => get_the_title( '<h1 class="tribe-events-single-event-title">', '</h1>' ),
		) );
